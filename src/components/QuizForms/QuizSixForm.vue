<template>
  <div>
    ...
    <div class="flex flex-col md:flex-row justify-center items-center gap-4 w-full">
      <label
        class="w-full flex items-center p-4 bg-white rounded-xl border border-slate-200 cursor-pointer hover:border-teal-600 hover:scale-[1.01] transition-all duration-300"
        :class="{ 'border-teal-600 bg-teal-50/50': selectedAnswer === 1 }"
      >
        <input type="radio" :name="'question'" :value="1" v-model="selectedAnswer" class="hidden" />
        <span
      ...
      >
        <input type="radio" :name="'question'" :value="0" v-model="selectedAnswer" class="hidden" />
        <span
          class="w-5 h-5 border rounded-full flex items-center justify-center mr-3"
